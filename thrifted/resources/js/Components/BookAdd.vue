...
async function SaveBook() {
    if (InputTitle.value == '' || InputCategory.value == 0 || InputAuthor.value == '' || SelectedTags.value == [] || InputFormat.value == '' || InputCondition.value == '' || InputState.value == '') {
...
<template>
    ...
    <Dialog v-model:visible="visible" modal header="Edit Profile" :style="{ width: '40rem' }">
        ...
        <div class="flex items-center gap-4 mb-2">
            ...
            <MultiSelect v-model="SelectedTags" display="chip" :options="props.tags" optionLabel="name" optionValue="id"
                filter placeholder="Select Tags" :maxSelectedLabels="3" class="flex-auto" />
        </div>
        <div class="flex items-center gap-4 mb-2">
            <label for="ssn" class="w-24 font-semibold">Format</label>
            <SelectButton v-model="InputFormat" :options="FormatOptions" aria-labelledby="basic" class="flex-auto" />
        </div>
        <div class="flex items-center gap-4 mb-2">
        ...
        <div class="flex items-center gap-4 mb-2">
            ...
            <SelectButton v-model="InputState" :options="StateOptions" aria-labelledby="basic" class="flex-auto" />
        </div>
        <div class="flex items-center gap-4 mb-2">
